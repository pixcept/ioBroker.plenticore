...
function calcMinSoC(forecastRead, tomorrow) {
    ...
    if(powerHours > 0) {
        ...
        for(let p = powerHours; p > 0; p--) {
			let shindex = Math.ceil(sun_hours - p) + 'h';
            ...
            if(fcPower) {   
				...
				// check if period is only partially relevant due to sunrise or sunset
				timeDiff = sunset.getTime() - time;
				Math.min(factor, 1 - (timeDiff / 3600000)); // take the smaller one: the current factor or the factor up to sunset   
				
				// TODO: case if timeNow is between time and sunset
				adapter.log.debug('Factor for remaining power:  start of period (timeTest); ' + time + '; factor; ' + factor);
