...
function readForecastData(callback) {
	...
	for(let i = 0; i < 50; i++) {
		...
		if(null !== fc) {
			...
			forecastHours[hindex]['rainData']['metno'] = fc.rain;
		...
		// calc averages
		for(let idx of ['cloudCover', 'time', 'rain', 'visibility']) {
			let tmpValues = [];
			...
			let tmpAvg = getArrayAvg(tmpValues);
			let tmpMin = getArrayMin(tmpValues);
			let tmpMax = getArrayMax(tmpValues);
			let newValues = [];

			if(idx !== 'time') {
				let spread = tmpMax - tmpMin;
				for(let i = 0; i < tmpValues.length; i++) {
					if(Math.abs(tmpValues[i] - tmpAvg) < (spread / 2) + 0.001) {
						newValues.push(tmpValues[i]);
						//adapter.log.debug('Keeping value ' + tmpValues[i] + ' in ' + idx+'Data' + ' because avg is ' + tmpAvg + ' (' + tmpMin + '/' + tmpMax + ').');
					} else {
						//adapter.log.debug('Removing value ' + tmpValues[i] + ' from ' + idx+'Data' + ' because avg is ' + tmpAvg + ' (' + tmpMin + '/' + tmpMax + ').');
					}
				}

				tmpAvg = getArrayAvg(newValues);
			}
...
function calcMinSoC(forecastRead, tomorrow) {
    ...
    while(powerTime.getTime() <= sunset.getTime()) {
		...
		let hindex = powerTime.getDate() + 'd' + sun_hour + 'h';
        ...
        if(wfc_fcdate - powerTime > 1800000) {
            ...
            if(wfc_curh <= 48) {
				...
				forecastData[hindex].time = powerTime.getTime();
            }
        }
	...
	if(tomorrow) {
		return;
	} else if(!adapter.config.enable_minsoc) {
			...
			for(let i in json[j][dataname]) {
				...
				if(mappings[setting.id]) {
					...
					if(objid === 'devices.local.ac.P') {
						...
						set_grid_p = true;
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeDay') {
						energy_home['Day'] = setting.value;
						set_yield_grid = 'Day';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeMonth') {
						energy_home['Month'] = setting.value;
						set_yield_grid = 'Month';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeYear') {
						energy_home['Year'] = setting.value;
						set_yield_grid = 'Year';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeTotal') {
						energy_home['Total'] = setting.value;
						set_yield_grid = 'Total';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeGridDay') {
						energy_home_grid['Day'] = setting.value;
						set_yield_grid = 'Day';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeGridMonth') {
						energy_home_grid['Month'] = setting.value;
						set_yield_grid = 'Month';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeGridYear') {
						energy_home_grid['Year'] = setting.value;
						set_yield_grid = 'Year';
					} else if(objid === 'scb.statistic.EnergyFlow.EnergyHomeGridTotal') {
						energy_home_grid['Total'] = setting.value;
						set_yield_grid = 'Total';
					...
					} else if(set_yield_grid !== false && energy_yield[set_yield_grid] !== null && energy_home[set_yield_grid] !== null && energy_home_grid[set_yield_grid] !== null) {
						grid_p = Math.round(energy_yield[set_yield_grid] + energy_home_grid[set_yield_grid] - energy_home[set_yield_grid]);
						if(grid_p > 0) {
