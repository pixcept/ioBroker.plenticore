...
function setObjects() {
	...
	adapter.setObjectNotExists('devices.local.battery.ExternControl_DcPowerAbs', {
		...
		common: {
			...
			def: 0,
			desc: {
				en: 'DANGER:Use this only if you know what you do, wrong usage can damage your battery! This state is only available if ExternControl is set to 2 (Modbus TCP). The value is in Watt and can be set from -10000 to 10000. A negative value means that the battery is discharging, a positive value means that the battery is charging.',
				de: 'GEFAHR: Verwenden Sie dies nur, wenn Sie wissen, was Sie tun, falsche Verwendung kann Ihre Batterie beschädigen! Dieser Zustand ist nur verfügbar, wenn ExternControl auf 2 (Modbus TCP) eingestellt ist. Der Wert ist in Watt und kann von -10000 bis 10000 eingestellt werden. Ein negativer Wert bedeutet, dass die Batterie entlädt, ein positiver Wert bedeutet, dass die Batterie lädt.',
				ru: 'ОПАСНОСТЬ: используйте это только если знаете что делаете, неправильное использование может повредить вашу батарею! Это состояние доступно только при установке ExternControl в 2 (Modbus TCP). Значение в ваттах может быть установлено от -10000 до 10000. Отрицательное значение означает, что батарея разряжается, положительное значение означает, что батарея заряжается.',
				pt: 'PERIGO: use isso apenas se souber o que está fazendo, o uso incorreto pode danificar sua bateria! Este estado só está disponível se ExternControl estiver definido como 2 (Modbus TCP). O valor está em Watt e pode ser definido de -10000 a 10000. Um valor negativo significa que a bateria está descarregando, um valor positivo significa que a bateria está carregando.',
				nl: 'GEVAAR: gebruik dit alleen als u weet wat u doet, verkeerd gebruik kan uw batterij beschadigen! Deze status is alleen beschikbaar als ExternControl is ingesteld op 2 (Modbus TCP). De waarde is in Watt en kan worden ingesteld van -10000 tot 10000. Een negatieve waarde betekent dat de batterij wordt ontladen, een positieve waarde betekent dat de batterij wordt opgeladen.',
				fr: 'DANGER: utilisez ceci uniquement si vous savez ce que vous faites, une mauvaise utilisation peut endommager votre batterie! Ce statut n\'est disponible que si ExternControl est défini sur 2 (Modbus TCP). La valeur est en Watt et peut être définie de -10000 à 10000. Une valeur négative signifie que la batterie se décharge, une valeur positive signifie que la batterie se charge.',
				it: 'PERICOLO: utilizzare questo solo se si sa cosa si sta facendo, un uso errato può danneggiare la batteria! Questo stato è disponibile solo se ExternControl è impostato su 2 (Modbus TCP). Il valore è in Watt e può essere impostato da -10000 a 10000. Un valore negativo significa che la batteria si scarica, un valore positivo significa che la batteria si carica.',
				es: 'Peligro: ¡utilice esto solo si sabe lo que está haciendo, el uso incorrecto puede dañar su batería! Este estado solo está disponible si ExternControl está configurado en 2 (Modbus TCP). El valor está en vatios y puede establecerse de -10000 a 10000. Un valor negativo significa que la batería se descarga, un valor positivo significa que la batería se carga.',
				pl: 'NIEBEZPIECZEŃSTWO: używaj tego tylko wtedy, gdy wiesz, co robisz, nieprawidłowe użycie może uszkodzić twoją baterię! Ten stan jest dostępny tylko wtedy, gdy ExternControl jest ustawiony na 2 (Modbus TCP). Wartość jest w watcie i może być ustawiona od -10000 do 10000. Wartość ujemna oznacza, że bateria się rozładowuje, wartość dodatnia oznacza, że bateria się ładowa.',
				'zh-cn': '危险：仅在您知道自己在做什么时才能使用，错误的使用可能会损坏您的电池！此状态仅在ExternControl设置为2（Modbus TCP）时可用。值以瓦特为单位，范围为-10000至10000。负值表示电池正在放电，正值表示电池正在充电。',
				uk: 'НЕБЕЗПЕКА: використовуйте це тільки тоді, коли ви знаєте, що робите, неправильне використання може пошкодити вашу батарею! Цей стан доступний тільки при налаштуванні ExternControl на 2 (Modbus TCP). Значення в ваттах може бути встановлено від -10000 до 10000. Від\'ємне значення означає, що батарея розряджається, позитивне значення означає, що батарея заряджається.'
			}
		},
		native: {}
	});
	...
	adapter.setObjectNotExists('devices.local.battery.ExternControl_MaxChargePowerAbs', {
		...
		common: {
			...
			def: 0,
			des: {
				en: 'DANGER: Use this only if you know what you are doing, wrong use can damage your battery! This state is only available if ExternControl is set to 2 (Modbus TCP).',
				de: 'GEFAHR: Verwenden Sie dies nur, wenn Sie wissen, was Sie tun, falsche Verwendung kann Ihre Batterie beschädigen! Dieser Status ist nur verfügbar, wenn ExternControl auf 2 (Modbus TCP) eingestellt ist.',
				ru: 'ОПАСНОСТЬ: используйте это только если знаете что делаете, неправильное использование может повредить вашу батарею! Это состояние доступно только при установке ExternControl в 2 (Modbus TCP).',
				pt: 'PERIGO: use isso apenas se souber o que está fazendo, o uso incorreto pode danificar sua bateria! Este estado só está disponível se ExternControl estiver definido como 2 (Modbus TCP).',
				nl: 'GEVAAR: gebruik dit alleen als u weet wat u doet, verkeerd gebruik kan uw batterij beschadigen! Deze status is alleen beschikbaar als ExternControl is ingesteld op 2 (Modbus TCP).',
				fr: 'DANGER: utilisez ceci uniquement si vous savez ce que vous faites, une mauvaise utilisation peut endommager votre batterie! Ce statut n\'est disponible que si ExternControl est défini sur 2 (Modbus TCP).',
				it: 'PERICOLO: utilizzare questo solo se si sa cosa si sta facendo, un uso errato può danneggiare la batteria! Questo stato è disponibile solo se ExternControl è impostato su 2 (Modbus TCP).',
				es: 'Peligro: ¡utilice esto solo si sabe lo que está haciendo, el uso incorrecto puede dañar su batería! Este estado solo está disponible si ExternControl está configurado en 2 (Modbus TCP).',
				pl: 'NIEBEZPIECZEŃSTWO: używaj tego tylko wtedy, gdy wiesz, co robisz, nieprawidłowe użycie może uszkodzić twoją baterię! Ten stan jest dostępny tylko wtedy, gdy ExternControl jest ustawiony na 2 (Modbus TCP).',
				'zh-cn': '危险：仅在您知道自己在做什么时才能使用，错误的使用可能会损坏您的电池！此状态仅在ExternControl设置为2（Modbus TCP）时可用。',
				uk: 'НЕБЕЗПЕКА: використовуйте це тільки тоді, коли ви знаєте, що робите, неправильне використання може пошкодити вашу батарею! Цей стан доступний тільки при налаштуванні ExternControl на 2 (Modbus TCP).'
			}
		},
		native: {}
	});
