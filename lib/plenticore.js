...
function readForecastData(callback) {
	...
	for(let i = 0; i < 36; i++) {
		...
		if(null !== fc) {
			forecastHours[hindex]['cloudCoverData']['kachelmann_suihd'] = getSkyMultiplier(fc.cloud_details);
			if(forecastHours[hindex]['cloudCover'] !== null) {
				forecastHours[hindex]['cloudCover'] = (forecastHours[hindex]['cloudCoverData']['kachelmann'] + forecastHours[hindex]['cloudCoverData']['kachelmann_suihd']) / 2;
			} else {
				forecastHours[hindex]['cloudCover'] = forecastHours[hindex]['cloudCoverData']['kachelmann_suihd'];
			}
			
			forecastHours[hindex]['timeData']['kachelmann_suihd'] = fc.time;
			if(forecastHours[hindex]['time'] !== null) {
				forecastHours[hindex]['time'] = (forecastHours[hindex]['timeData']['kachelmann'] + forecastHours[hindex]['timeData']['kachelmann_suihd']) / 2;
			} else {
				forecastHours[hindex]['time'] = forecastHours[hindex]['timeData']['kachelmann_suihd'];
			}
			
			forecastHours[hindex]['rainData']['kachelmann_suihd'] = fc.rain;
			if(forecastHours[hindex]['rain'] !== null) {
				forecastHours[hindex]['rain'] = (forecastHours[hindex]['rainData']['kachelmann'] + forecastHours[hindex]['rainData']['kachelmann_suihd']) / 2;
			} else {
				forecastHours[hindex]['rain'] = forecastHours[hindex]['rainData']['kachelmann_suihd'];
			}
		}
	}
...
function calcMinSoC(forecastRead) {
    ...
    for(let p = 0; p < power.length; p++) {
		...
		fc_state_id = 'forecast.power.' + sunhours[p] + 'h.power_high';
        ioBLib.createOrSetState(fc_state_id, {
			type: 'state',
			common: {
