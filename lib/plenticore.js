		...
		if(json[j][dataname]) {
			for(let i in json[j][dataname]) {
				...
				if(mappings[setting.id]) {
					...
					if(objtype === 'boolean') {
						adapter.log.info('Converting ' + setting.value + ' to bool for ' + objid);
						setting.value = (setting.value === 1 || setting.value === '1');
					} else if(objtype === 'int') {
						adapter.log.info('Converting ' + setting.value + ' to int for ' + objid);
						setting.value = parseInt(setting.value);
					} else if(objtype === 'float') {
						adapter.log.info('Converting ' + setting.value + ' to float for ' + objid);
