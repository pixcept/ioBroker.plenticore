...
function calcMinSoC(forecastRead, tomorrow) {
    ...
    if(powerHours > 0) {
        ...
        for(let p = powerHours; p > 0; p--) {
			...
			if(!tomorrow) {
				if(fcPower > panelWp * maxFeedInFactor) {
		...
		adapter.getState('forecast.consumption.day', function(err, state) {
			...
			adapter.getState('devices.local.battery.SoC', function(err, state) {
				...
				adapter.getState(useMinSoCId, function(err, state) {
					...
					adapter.log.debug('Checking battery management: ' + power_plus + ' = ' + powerUntilSunset + ' - ' + daily_cons + ' + ' + curSoC + ' - ' + adapter.config.battery_capacity + ' > ' + panelWp + ' * ' + maxFeedInFactor + ' (' + (panelWp * maxFeedInFactor) + ') -> ' + fc_above_70);
