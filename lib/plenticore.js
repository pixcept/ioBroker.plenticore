...
function init(adapterInstance, utilsInstance, weatherInstance) {
	...
	adapter.log.info('Configured Wp of panel(s) is ' + panelWp)
}
...
function calcMinSoC(forecastRead, tomorrow) {
	...
	if(doMinSoC === true) {
		adapter.log.debug('Using this forecast day for MinSoC calculation.');
	}
    ...
    for(let p = 0; p < power.length; p++) {
        ...
        if('undefined' !== typeof skyvis[p] && skyvis[p] !== null) {
            vis_multiplier = getVisMultiplier(skyvis[p]);
        } else {
            vis_multiplier = getVisMultiplier(visavg);
        }
