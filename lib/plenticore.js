...
function getSkyMultiplier(sky) {
	...
	multisky = 1 - (0.93 * Math.pow(sky, 1.1));
...
function getRainMultiplier(rain) {
	...
	if(rain > 0) {
		multiplier = rain / 10;
		multiplier = 1 - (0.5 * Math.pow(multiplier, 1.1));
		if(multiplier < 0.3) {
			multiplier = 0.3;
		}
	}
...
function calcMinSoC(forecastRead, tomorrow) {
	...
	to_delete.forEach(function(id) {
		adapter.getObject(id, function(err, obj) {
			if(!err && obj) {
				adapter.delObject(obj.id);
				adapter.log.info(obj.id + ' deleted.');
			}
		});
	});
    ...
    if(rotate) {
		...
		if(generationData.hours) {
			...
			for(let hidx in generationData.hours) {
				...
				adapter.getObject(delId, function(err, obj) {
					if(!err && obj) {
						adapter.delObject(obj.id);
						adapter.log.info(obj.id + ' deleted.');
					}
