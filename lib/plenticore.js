    ...
    if(powerHours > 0) {
		...
		for(let shindex in sunhour_power) {
			...
			if(fcPower) {   
				...
				// TODO error handling in case of null
				...
				// check if period is only partially relevant due to sunset
				timeDiff = sunset.getTime() - Math.max(time, timeNow.getTime());  // take either the diff between start of the power or the current time. The "bigger" (=later) one is the more relevant one.
				factor = Math.min(factor, 1 - (timeDiff / 3600000));
				factor = Math.max(factor, 0); // ensure that the factor is never negative
				// sum up the number of power hours remaining
				adapter.log.debug('Factor for powerHours for remaining power:  start of period; ' + time + '; factor; ' + factor + '  shindex: ' + shindex + '  sunset: ' + sunset.getTime());
				powerHours += factor;  
