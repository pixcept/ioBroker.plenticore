	...
	if(adapter.config.wfc_instance) {
		if(useInternalForecast === true) {
			...
			if(kmFc) {
				...
				if(internalForecastType === 'clouds') {
					let pLow = 1 - (kmFc.low / 100);
					let pMed = 1 - (kmFc.medium / 100);
					let pHigh = 1 - (kmFc.high / 100);
					
					setValue = 100 - (100 * pLow * pMed * pHigh) + ((kmFc.low + kmFc.medium + kmFc.high) / 30);
				} else {
...
function readForecastData(callback) {
	if(useInternalForecast) {
		...
		for(let i = 0; i < 36; i++) {
			...
			if(null !== fc) {
				if(internalForecastType === 'clouds') {
					let pLow = 1 - (fc.low / 100);
					let pMed = 1 - (fc.medium / 100);
					let pHigh = 1 - (fc.high / 100);
					
					setValue = 100 - (100 * pLow * pMed * pHigh) + ((fc.low + fc.medium + fc.high) / 30);
				} else {
