...
// TODO: Strategy 2					
					adapter.log.info('Checking battery management: power_plus < panelWp * maxFeedInFactor: ' + power_plus + ' < ' + panelWp + ' * ' + maxFeedInFactor + ' (enable_smart_battery_control was before: ' + enable_smart_battery_control);
					if(power_plus <= panelWp * maxFeedInFactor) {
						adapter.log.info('enable_smart_battery_control set to false; reason: power_plus too low');
						enable_smart_battery_control = false;
					} else if(curSoCPercentage < adapter.config.enable_bm_minsoc) {
						adapter.log.info('enable_smart_battery_control set to false; reason: curSoCPercentage < adapter.config.enable_bm_minsoc: ' + curSoCPercentage + ' < ' + adapter.config.enable_bm_minsoc);
						enable_smart_battery_control = false;
					}
/* TODO: Strategy 1					
