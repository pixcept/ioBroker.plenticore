...
function getSkyMultiplier(sky) {
	...
	multisky = 1 - (0.92 * Math.pow(sky, 1.2));
...
function getForecastDataFor(time) {
	...
	for(let i = 0; i < 50; i++) {
		...
		for(let dataIdx of ['visibility','cloudCover','rain']) {
			for(let idx in forecastHours[hindex][dataIdx+'Data']) {
				...
				adapter.log.info('Matching hindex for time ' + time + ' is ' + hindex + ' of ' + idx + ' (' + (new Date(forecastHours[hindex]['timeData'][idx])) + ')');
			}
		}
	}
...
function calcMinSoC(forecastRead, tomorrow) {
	...
	minSoCDate.setMinutes(minSoCDate.getMinutes() + 120);
	...
	adapter.log.info('Calc power from ' + powerTime + ' to ' + sunset + ' now.');
    while(powerTime.getTime() <= sunset.getTime()) {
		...
		if(fcData['cloudCover'] === null) {
			...
			if(fcData['cloudCover'] === null) {
				fcData['cloudCover'] = 1; // multiplier
			}
		}
		...
		adapter.log.debug('Multipliers for ' + powerTime + ' (' + pwr.panelpower + '): ' + skyMulti + '/' + visMulti + '/' + rainMulti);

		let skyPower = pwr.panelpower * skyMulti;
		let skyVisPower = skyPower * visMulti;
		let skyRainPower = skyPower * visMulti * rainMulti;
		...
		powerTime.setMinutes(powerTime.getMinutes() + 5);
	}
	...
	let fcDay = new Date(sunhour_starts['0h']);
	...
	if(!tomorrow && adapter.config.enable_battery_management) {
		...
		adapter.getState('forecast.consumption.day', function(err, state) {
			...
			adapter.getState('devices.local.battery.SoC', function(err, state) {
				if(!err) {
					curSoC = state.val;
					curSoC = curSoC * adapter.config.battery_capacity / 100;
				}

				let power_plus = powerUntilSunset - daily_cons + curSoC - adapter.config.battery_capacity;
				adapter.log.debug('Checking battery management: ' + power_plus + ' = ' + powerUntilSunset + ' - ' + daily_cons + ' + ' + curSoC + ' - ' + adapter.config.battery_capacity + ' > ' + panelWp + ' * 0.7 (' + (panelWp * 0.7) + ')');
