		...
		if(useInternalForecast === true) {
			...
			if(kmFc) {
				...
				if(internalForecastType === 'clouds') {
					...
					ioBLib.createOrSetState('forecast.current.sky_low', {
						type: 'state',
						common: {
					...
					ioBLib.createOrSetState('forecast.current.sky_medium', {
						type: 'state',
						common: {
					...
					ioBLib.createOrSetState('forecast.current.sky_high', {
						type: 'state',
						common: {
...
function calcMinSoC(forecastRead) {
    ...
    for(let p = 0; p < power.length; p++) {
		...
		if(typeof cursky_value === 'object') {
			...
			multiplier = m1 * m2 * m3;*/
			let m1 = 1 - (cursky_value.low / 100);
			let m2 = 1 - (cursky_value.medium / 100);
			let m3 = 1 - (cursky_value.high / 100);
			
			let msum = cursky_value.low + cursky_value.medium + cursky_value.high;
			if(msum < 100) {
				msum = 100;
			}

			let msky = 1 - (m1 * m2 * m3);
			multiplier = 1 - (0.75 * Math.pow(msky, 1.5));
			multiplier = Math.pow(multiplier, msum/100);
		} else {
