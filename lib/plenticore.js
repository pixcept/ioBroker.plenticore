...
function storeSunPanelSkyData(panel_power, sky, vis) {
    let multisky = (1 - (Math.pow(sky, 1.8) / 4500));
    if(multisky < 0.05) {
        multisky = 0.05;
    }
	...
	if(adapter.config.wfc_instance) {
		if(useInternalForecast === true) {
			...
			if(kmFc) {
				...
				if(internalForecastType === 'clouds') {
					let pLow = kmFc.low / 100;
					let pMed = kmFc.medium / 100;
					
					setValue = kmFc.low + (pLow * kmFc.medium) + (pLow * pMed * kmFc.high);
				} else {
					setValue = Math.round(100 - (100 * (kmFc.sun - ((180 - (kmFc.sun * 3)) / 60)) / 60));
				}
	...
	if(useInternalForecast) {
		...
		for(let i = 0; i < 36; i++) {
			...
			if(null !== fc) {
				if(internalForecastType === 'clouds') {
					let pLow = fc.low / 100;
					let pMed = fc.medium / 100;
					
					setValue = fc.low + (pLow * fc.medium) + (pLow * pMed * fc.high);
				} else {
					setValue = Math.round(100 - (100 * (fc.sun - ((180 - (fc.sun * 3)) / 60)) / 60));
				}
...
function calcMinSoC(forecastRead) {
    ...
    for(let p = 0; p < power.length; p++) {
        ...
        let vis_multiplier;
        if(sky[p] !== null) {
            multiplier = (1 - (Math.pow(sky[p], 1.8) / 4500));
        } else {
            multiplier = (1 - (Math.pow(skyavg, 1.8) / 4500));
        }
