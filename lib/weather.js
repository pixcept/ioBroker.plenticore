...
function parseMetnoData(json, callback) {
	...
	for(let i = 0; i < forecastArr.length; i++) {
		...
		if(entry.data.next_1_hour && 'precipitation' in entry.data.next_1_hour) {
			fc_rain = parseFloat(entry.data.next_1_hour.precipitation);
		} else if(entry.data.next_6_hours && 'precipitation' in entry.data.next_6_hours) {
			fc_rain = parseFloat(entry.data.next_6_hours.precipitation);
		}
